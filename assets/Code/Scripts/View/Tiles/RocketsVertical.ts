...
@ccclass
export default class RocketsVertical extends Rockets {
            ...
            .call(() => {
                ...
                for (let i = 0; i < 2; i++) {
                    new cc.Tween(rockets[i])
                        .by(rocketFlyTime, { y: distance * multipliers[i] })
                        .start();
                }

                for (let i = 1; i < height; i++) {
                    for (const multiplier of multipliers) {
                        const y = position.y + i * multiplier;
                        if (y < 0 || y >= height) continue;
                        const time = i * (view.tileSize.y / view.tileSpeed);
                        maxDestroyDelay = Math.max(maxDestroyDelay, time);
                        view.scheduleOnce(() => {
                            const tileView = field[position.x][y];
                            if (!tileView?.config) return;
                            if (tileView.config.tile.type === 'bonus') return;
                            tileView.animateDestroy();
                            field[position.x][y] = null;
                        }, time);
                    }
                }
            })
